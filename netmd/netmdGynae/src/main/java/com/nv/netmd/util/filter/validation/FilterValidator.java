 ...
 */
package com.nv.netmd.util.filter.validation;
...
public class FilterValidator {
	...
	/**
	 * Validate expression and return error, if expression name,value or
	 * operator is invalid
	 * 
	 * @param exp
	 * @param property 
	...
	protected ErrorDTO validateExp(ExpressionDTO exp, Property property) {
		...
		if (!isValidExpOperator(exp.getOperator())) {
			...
			error.setErrCode(ErrorCodeEnum.InvalidFilterExpOperator
					.getErrCode());
			List<Parameter> params = new ArrayList<Parameter>();
			Parameter param = new Parameter();
	...
	private boolean isValidExpValue(String value, Property property) {
		...
		if (value != null && !value.equals("")) {
			value1 = value.trim();
			Class<?> type = PropertyUtil.getFieldType(property.getFieldName(),
					property.getEntityName());
			if (PropertyUtil.isValidPropertyValue(type, value1)) {
				return true;
			}
		}
		return false;
