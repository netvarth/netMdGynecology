...
(function (m, oa) {
    ...
    function kb(a, b) {
        ...
        if (typeof a == "string") {
            if (a.match(/^\d+(\.\d+)?$j/)) return new Date(parseFloat(a) * 1E3);
            if (b === oa) b = true;
    ...
    function Bb(a, b) {
        a = a.match(/^([0-9]{4})(-([0-9]{2})(-([0-9]{2})([T ]([0-9]{2}):([0-9]{2})(:([0-9]{2})(\.([0-9]+))?)?(Z|(([-+])([0-9]{2})(:?([0-9]{2}))?))?)?)?)?$j/);
        if (!a) return null;
    ...
    function sb(a, b, e) {
        ...
        var L = p.opt,
            ...
            ya = b.formatDate,
            K, i, C, P, E, B, n, Y, W, o, s, v, F, r, J, M, k, D, Z, ja, ia, la, $j;
    ...
    function Rb(a, b, e) {
        ...
        function g() {
            ...
            for (A = tb = 0; ta < za; A++) {
                Da = ta.getMinutes();
                x += "<tr style='height:40px'class='fc-slot " + A + " " + (!Da ? "" : "fc-minor") + "'><th class='fc-agenda-axis " + h + "'>" + (!Ea || !Da ? s(ta, i("axisFormat")) : s(ta, i("axisFormat"))) + "</th><td class='" + O + "'><div style='position:relative'>&nbsp;</div></td></tr>";
                xa(ta, i("slotMinutes"));
                tb++
            }
            x += "</tbody></table>";
        ...
        var i = K.opt,
            ...
            o = K.slotSegHtml,
            s = b.formatDate,
            v, F, r, J, M, k, D, Z, ja, ia, la, $j, w, I, R, V, ea, aa, va, wa, Ga, Wb, Ma, db, vb, Xa, Xb, Ba, tb, Na, Ra, cb, ub = {}, Wa, Tb, Sb, Ub, Ha, Ia, La, bb, Vb;
    ...
    function sc() {
        ...
        function g(o, s) {
            ...
            for (v = 0; v < F; v++) {
                ...
                if ($j = r.element) {
                    J = R[s = r.key = Ib($j[0])];
                    r.vsides = J === oa ? (R[s] = Sa($j, true)) : J;
                    J = V[s];
                    r.hsides = J === oa ? (V[s] = pb($j, true)) : J;
                    s = $j.find("div.fc-event-content");
            ...
            for (v = 0; v < F; v++) {
                r = o[v];
                if ($j = r.element) {
                    $j[0].style.width = Math.max(0, r.outerWidth - r.hsides) + "px";
                    R = Math.max(0, r.outerHeight - r.vsides);
                    $j[0].style.height = R + "px";
                    J = r.event;
                    if (r.contentTop !== oa && R - r.contentTop < 10) {
                        $j.find("div.fc-event-time").text(Y(J.start, u("timeFormat")) + " - " + J.title);
                        $j.find("div.fc-event-title").remove()
                    }
                    fa("eventAfterRender", J, J, $j)
                }
            }
        }
        function l(o, s) {
            ...
            n.mousedown(function (W) {
                ...
                if (W.which == 1) {
                    ...
                    s.start(function (ia, la) {
                        if (ia) {
                            var $j = Math.max(ja.row, ia.row);
                            ia = ia.col;
                            if (v == 1) $j = 0;
                            if ($j == ja.row) ia = E ? Math.min(ja.col, ia) : Math.max(ja.col, ia);
                            k = $j * 7 + ia * r + J - (la.row * 7 + la.col * r + J);
                            la = ba(sa(i), k, true);
                            if (k) {
                                Z.end = la;
                                $j = D;
