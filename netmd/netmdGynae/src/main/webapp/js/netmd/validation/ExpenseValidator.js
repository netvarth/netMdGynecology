function ExpenseValidator() {
	...
	this.validate = function(expense) {
		...
		else if(validator.validateLimit(expense.getExpenseName())) {
			error.setErrorStatus(true);
			var errorMessage = new ErrorMessageDTO(constants.NAME,constants.FIELDEXCEEDLIMIT);
			errorMsgs.push(errorMessage);
		} else if(!validator.validateName(expense.getExpenseName())) {
			error.setErrorStatus(true);
