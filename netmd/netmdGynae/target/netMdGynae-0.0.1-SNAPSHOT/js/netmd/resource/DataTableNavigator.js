...
DataTableNavigator.prototype.list = function() {
	...
	if(self.pgDataList.caseList){
	var to=self.filterDTO.from + self.pgDataList.caseList.length;
	var info = "Showing " + (self.filterDTO.from+1) + " to " + to + " of " + self.pgDataList.count;
	jQuery('.dataTables_info').html(info); 
	}
